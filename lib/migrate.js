...
function Migrate (onlineRequest, localRequest, localDomain, fromDomain) {
  this.onlineGroup = ''
  this.localGroup = ''
...
Migrate.prototype.getMigrateType = function getMigrateType () {
  inquirer
    .prompt([
      {
        type: 'list',
        message: `选择迁出项目的类型（${ this.fromDomain }）`,
        name: 'migrateType',
        choices: [
          { name: '个人项目', value: 'personal' },
          { name: '团队项目', value: 'team' }
...
Migrate.prototype.getGroup = function getGroup (isOnline) {
  ...
  const msg = isOnline ? `选择团队（${ this.fromDomain }）` : `选择团队（${this.localDomain}）`
  request
    .get('/group')
    .then(res => {
  ...
  this.onlineRequest
    ...
    .then(res => {
      ...
      if (body && body.length) {
        return inquirer.prompt([{
          type: 'checkbox',
          message: `选择要迁出的项目（${ this.fromDomain }）`,
          name: 'projects',
